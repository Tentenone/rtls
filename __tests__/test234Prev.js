...
const trilat234Prev = require('../trilat234Prev')
...
'     - for the case of 4 received points', () => {
  ...
  let pos = trilat234Prev(signals, {z: 1.5}, currentPoint)
  if (pos.isInside === 1) {
...
test('should have same z as target for 3 points', () => {
  ...
  let results = trilat234Prev(makeSignals(ROOM, [0, 3.15, 6.04, 5.87]).slice(1, 4), {z}, currentPoint)
  expect(results.z).toBe(1.7)
})

test('should have same z as target for 2 points', () => {
  let z = 1.6
  let currentPoint = {x: 30.510871243, y: 50.437567342, z: 1.6}
  let results = trilat234Prev(makeSignals(ROOM, [0, 0, 3.49, 5.93]).slice(2, 4), {z}, currentPoint)
...
test('should return "undefined" for 1 points', () => {
  ...
  let pos = trilat234Prev(signals, {z: 0}, currentPoint)
...
test('should return "undefined" for 0 points', () => {
  let currentPoint = {x: 30.510871243, y: 50.437567342, z: 1.5}
  let pos = trilat234Prev([], {z: 0}, currentPoint)
  // ensure that pos is undefined
  expect(pos).toBe(undefined)
})
