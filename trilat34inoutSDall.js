// function trilat234outSDall (points, target) takes:
...
// function trilat234outSDall refers to the function trilateration which calculates all trilateration points and
// trilat234outSDall choose from these points (using points for all pairs, triples and quadruples of signals)
// trilateration point with minimal sumDevAll (with minimal sum of deviations from all 3 or 4 trilateration
// circles) inside the room tested;
// if there is no point within the room tested then returns point outside the room with minimal sumDevAll
// if there is 2, 1 or 0 signals received then returns undefined
// function trilat234outSDall returns:
...
function trilat234outSDall (points, target) {
  if (points.length < 3) {
    return undefined
  }
  ...
  for (let j = 0; j < resultPoints.length; j++) {
    if (resultPoints[j].isInside === -1) {
      resultPoints[j].z = target.z
      return resultPoints[j]
    }
  }
}
module.exports = trilat234outSDall
